...
const { log, error, throwError, createMongooseObject, sendData, freezeObject } = require('../lib/utils');

const defaultPictureProfile = `http://${process.env.host}:${process.env.port}/people.png`;

// CRUD : CREATE READ UPDATE DELETE̋
...
const createUserObject = (username, password, createdAt = new Date(), verified = false) => freezeObject({
  username,
  password,
  createdAt,
  verified
...
const createUserInfoObject = (firstname, lastname, email, createdAt = new Date(), profilePicture = defaultPictureProfile) => freezeObject({
  firstname,
  lastname,
  email,
  createdAt,
...
const throwEmailTakenError = res => throwError(res, 'Email already taken', 400);

const createUserObjectWithHashedPassword = (userObject, password) => freezeObject({ ...userObject, password });

const createUserInfoObjectWithUserId = (userInfoObject, userId) => freezeObject({ ...userInfoObject, userId });

const createMailUserDetail = (firstname, lastname, email) => freezeObject({ firstname, lastname, email });
    ...
    .then((users) => {
      if (users.length) {
        ...
        bcrypt.hash(userObj.password, 10, (err, hash) => {
          ...
          currentUser.save((errSaveUser, user) => {
            if (errSaveUser) {
              throwError(res, errSaveUser.message);
              error('createUser', 'users.crud.js:64', errSaveUser);
            }
            const currentUserInfo = createMongooseUserInfoObject(createUserInfoObjectWithUserId(userInfoObj, user._id));
            currentUserInfo.save((errSaveUserInfo, userInfo) => {
              if (errSaveUserInfo) {
                throwError(res, errSaveUserInfo.message);
                error('createUser', 'users.crud.js:71', errSaveUserInfo);
              }
              ...
              emails.sendVerificationMail(userDetail)
                .then((data) => {
                  updateManyUsers({ _id: user._id }, { $set: { token: { verify: data.hash } } })
                    .then((userUpdated) => {
                      sendData(res, userUpdated);
                    })
                    .catch((errUpdateUsers) => {
                      throwError(res, errUpdateUsers.message);
                      error('findUsers', 'users.crud.js:86', errUpdateUsers);
                    });
                })
                .catch((errMail) => {
                  throwError(res, errMail.message);
                  error('findUsers', 'users.crud.js:91', errMail);
                });
            });
          });
        });
        ...
        users.forEach((user) => {
          UserInfo.deleteOne({ userId: user._id }, (err) => {
            ...
            User.deleteOne({ _id: user._id }, (errDeleteUser) => {
              if (errDeleteUser) {
                throwError(res, errDeleteUser.message);
          ...
          if (err) {
            ...
            userInfosCrud.findUserInfos({ userId: user._id })
              ...
              .catch((errFindUserInfos) => {
                throwError(res, errFindUserInfos.message);
...
const setUsername = (req, res) => {
  ...
  if (username) {
    if (username.length < 4) {
      throwError(res, '4 character length minimum is required');
    }
    findUsers({ username })
      .then((users) => {
        if (users.length) {
